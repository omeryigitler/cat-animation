...
type CatType = 'default' | 'garfield' | 'cute';
type ColorTheme = 'orange' | 'calico' | 'gray' | 'black' | 'white';
type Accessory = 'none' | 'hat' | 'glasses' | 'bowtie';
type Phrase = { id: string; text: string; timeSec: number };
...
const STORAGE_KEY = 'berfin-dashboard-cat-settings-v1';
...
function readSettings(): CatSettings {
  try {
    const stored = window.localStorage.getItem(STORAGE_KEY);
    if (!stored) return DEFAULT_SETTINGS;

    const parsed = JSON.parse(stored) as Partial<CatSettings>;
    const phrases = Array.isArray(parsed.phrases)
      ? parsed.phrases
          .filter((phrase): phrase is Phrase => Boolean(phrase && typeof phrase.text === 'string'))
          .map((phrase) => ({
            id: String(phrase.id || crypto.randomUUID()),
            text: phrase.text,
            timeSec: Number.isFinite(Number(phrase.timeSec)) ? Math.max(1, Number(phrase.timeSec)) : 5,
          }))
      : DEFAULT_SETTINGS.phrases;
  ...
  return (
    ...
    >
      <span className="min-w-0 flex-1">
        ...
        <small className="mt-1 block text-[10px] font-semibold leading-relaxed text-gray-500">
          {description}
        </small>
      </span>
...
export default function EmbedApp() {
  ...
  const [settings, setSettings] = useState<CatSettings>(() => readSettings());
  ...
  const updateSettings = (patch: Partial<CatSettings>) => {
    setSettings((current) => ({ ...current, ...patch }));
  };

  const addPhrase = () => {
    updateSettings({
      phrases: [...settings.phrases, { id: crypto.randomUUID(), text: 'Miyav!', timeSec: 10 }],
    });
  };

  const updatePhrase = (id: string, patch: Partial<Phrase>) => {
    updateSettings({
  ...
  if (widgetMode) {
    return <div className="fixed inset-0 overflow-hidden bg-transparent pointer-events-none">{catLayer}</div>;
  }

  return (
    <main className="relative min-h-screen overflow-hidden bg-[#f6f5f1] p-5 text-[#292723] sm:p-7">
      <div className="mx-auto flex max-w-6xl flex-col gap-5 pb-36">
        <header className="flex flex-col gap-4 rounded-[2rem] border border-black/[0.07] bg-white/90 p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-[10px] font-semibold text-gray-500">Değişiklikler otomatik kaydedilir ve açık kediye uygulanır.</p>
            </div>
          </div>
          ...
          <section className="rounded-[2rem] border border-black/[0.07] bg-white/90 p-5 shadow-sm">
            ...
            <div className="space-y-5">
              ...
              <Toggle
                ...
                description="Kedi boşta kaldığında yönetim ekranında kendi kendine gezinir."
      ...
      <div className="pointer-events-none fixed inset-0 z-40 overflow-hidden">{catLayer}</div>
    </main>
  );
}
